...
- Restyled SPA with base path `/soc/` and deployed via Python backend
...
- `/home/aiagent/soc-ui/src/pages/Autopilot.jsx` — case list with AI confidence
- `/home/aiagent/soc-ui/src/pages/AutopilotCase.jsx` — case detail with AI analysis card

### Access
- SPA: `/soc` (port 8095, behind auth)
- Old HTML: `/soc.html` (fixed — was blank due to missing DOM elements + CSS vars)
- API: DeepSeek via `AI_API_URL` in `.env`

### Alert flow
SOC alerts (port 5000 SOC API) → AI triage (DeepSeek or rule fallback) → auto-resolve (level ≤7 or confident) or case creation → human approve/reject/execute

### Issues fixed
- Old soc.html: JS crash from missing DOM elements (#refreshBtn etc.) + CSS var mismatch
